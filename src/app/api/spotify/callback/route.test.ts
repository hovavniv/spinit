...
it("redirects using SPOTIFY_REDIRECT_URI's origin, ignoring BOTH request.url's host\n" +
   "(Next's dev proxy reports its own internal target, not the client-facing host) AND\n" +
   "the request's Host header (client-supplied -- trusting it for a redirect Location\n" +
   "is host-header injection on the most security-sensitive route in this slice)",
  async () => {
    ...
    // A malicious or merely-different Host header, deliberately distinct from
    // both the URL's own host and the configured redirect origin -- three
    // different values in play, and only one of them may legitimately win.
    const request = new Request(u, { headers: { host: 'evil.example.com' } });
